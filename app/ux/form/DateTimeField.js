Ext.define('vyl.ux.form.DateTimeField', {
    extend: 'Ext.form.field.Picker',
    alias: 'widget.datetimefield',

    // requires: ['Ext.picker.Date', 'Ext.picker.Time'],

    /**
     * @cfg {String} [format="d/m/Y H:i"]
    ...
    /**
     * @cfg {String} [ariaFormat="M j Y H:i"]
     ...
     * @locale
     */
    altFormats: 'd/m/Y|d/m/Y H:i|d/m/Y H:i:s|Y-m-d|Y-m-d H:i:s|Y-m-d\\TH:i:s',
    ...
    setDisabledDates: function (disabledDates) {
        var me     = this,
            picker = me.picker.getDatePicker();
    ...
    setDisabledDays: function (disabledDays) {
        var picker = this.picker.getDatePicker();
    ...
    safeParse: function (value, format) {
        return Ext.Date.parse(value, format, this.useStrict);
    },
    ...
    parseDate: function (value) {
        ...
        if (!val && altFormats) {
            var altFormatsArray = altFormats.split('|');
    ...
    createPicker: function () {
        ...
        return Ext.create('Ext.container.Container', {
            ...
            items: [{
                ...
                xtype: 'timepicker',
                width: 65,
                format: me.timeFormat || 'H:i',
                minValue: me.minTimeValue || Ext.Date.parse('08:00:00', 'H:i:s'),
                maxValue: me.maxTimeValue || Ext.Date.parse('20:00:00', 'H:i:s'),
                increment: me.timeInterval || 30,
                snapToIncrement: true,
                listeners: {
                    scope: me,
                    select: me.onSelectTime
                }
            }],
            getDatePicker: function () {
                return me.getPicker().items.getAt(0);
            },
            getTimePicker: function () {
                return me.getPicker().items.getAt(1);
            }
        });
    },
    ...
    onSelect: function (m, d) {
        var me  = this,
            old = me.getValue() ? me.getValue() : new Date();

        d = new Date(d.getFullYear(), d.getMonth(), d.getDate(), old.getHours(), old.getMinutes(), old.getSeconds(), 0);
    ...
    onSelectTime: function (m, d) {
        ...
        d = d.get('date'); //d is a model from timepicker
        d = new Date(old.getFullYear(), old.getMonth(), old.getDate(), d.getHours(), d.getMinutes(), d.getSeconds(), 0);
    ...
    onExpand: function () {
        var value      = this.rawDate,
            picker     = this.picker.getDatePicker(),
            timePicker = this.picker.getTimePicker();

        var toDate = Ext.isDate(value) ? value : this.createInitialDate();
        var toTime = new Date(2008, 0, 1, toDate.getHours(), toDate.getMinutes(), toDate.getSeconds(), 0);

        picker.setValue(toDate);
        var recIx = timePicker.getStore().findExact('date', toTime);
        if (recIx >= 0) {
            timePicker.suspendEvents();
            timePicker.getSelectionModel().select(recIx);
            timePicker.resumeEvents();
            timePicker.getScrollable().scrollIntoView(timePicker.getNode(recIx), true);
        }
        else {
            timePicker.suspendEvents();
            timePicker.getSelectionModel().deselectAll();
            timePicker.resumeEvents();
            //not found, so deselect all, but bring the closest one to view
            toTime = new Date(Ext.Number.snap(toTime.getTime(), timePicker.increment * 60 * 1000));
            if (toTime < timePicker.minValue) {
                recIx = 0;
            }
            else if (toTime < timePicker.maxValue) {
                recIx = timePicker.getStore().findExact('date', toTime);
            }
            else {
                recIx = timePicker.getStore().getCount() - 1;
            }
            timePicker.getScrollable().scrollIntoView(timePicker.getNode(recIx >= 0 ? recIx : 0), true);
        }
    },
